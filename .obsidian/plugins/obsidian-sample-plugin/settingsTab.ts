...
import { MapState, LegacyMapState } from "src/Map/src/mapState";
import * as consts from 'src/Map/src/consts';
import { DEFAULT_MAX_TILE_ZOOM, MAX_ZOOM } from "src/Map/src/consts";
import { getIconFromOptions, getIconFromRules } from "src/Map/src/markerIcons";
import { BaseMapView } from "src/Map/src/baseMapView";

type GeoHelperType = 'url' | 'app' | 'commandline';
type LegacyOpenBehavior = 'samePane' | 'secondPane' | 'alwaysNew';
type UrlParsingRuleType = 'latLng' | 'lngLat' | 'fetch';
type UrlParsingContentType = 'latLng' | 'lngLat' | 'googlePlace';
type LinkNamePopupBehavior = 'never' | 'always' | 'mobileOnly';
type MapLightDark = 'auto' | 'light' | 'dark';
...
type LegacyUrlParsingRule = UrlParsingRule & {
...
function convertLegacyMarkerIcons(settings: Settings): boolean {
...
function convertLegacyTilesUrl(settings: Settings): boolean {
...
function convertLegacyDefaultState(settings: Settings): boolean {
  if (
...
function removeLegacyPresets1(settings: Settings): boolean {
...
function convertTagsToQueries(settings: Settings): boolean {
  let changed = false;
...
function convertUrlParsingRules1(settings: Settings): boolean {
  let changed = false;
...
function convertLegacyOpenBehavior(settings: Settings): boolean {
  let changed = false;
...
export class SettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    containerEl.createEl('h1', {
      text: 'Settings for Map, AI, and Matrix plugin'
    });
    ...
    // Makes the other setting disappear for some reason
    ...
    /* AI Setting */
    containerEl.createEl("h3", { text: "Settings for my AI assistant plugin." });
    ...
    // text assistant
    containerEl.createEl("h2", { text: "Text Assistant" });
    ...
    // Image assistant
    containerEl.createEl("h2", { text: "Image Assistant" });
    ...
    /* Matrix Setting */
    containerEl.createEl("h3", { text: "Settings for my Matrix plugin." });
